...
func (srv *CategoryService)  Add(category model.CategoryResult) (bool, error) {
	...
	//判断是否存在,3个category有一个不存在都可以添加成功
	c1 := model.Category{
	...
	c2 := model.Category{
		...
		ParentId:category.C1CategoryID,
		IsDeleted: false,
	}
	r2 := srv.Exist(c2)
	...
	c3 := model.Category{
		...
		ParentId:category.C2CategoryID,
		IsDeleted: false,
	}
	r3 := srv.Exist(c3)

	if r1.Name != ""&&r2.Name!=""&&r3.Name!=""{
		return false,errors.New("分类已经存在")
	}
	if r1.Name ==""{
		srv.Repo.Add(c1)
	}
...
func (srv *CategoryService) Delete(c model.Category) (bool, error) {
	...
	category := srv.ExistByCategoryID(c.CategoryID) //返回的是指针类型
	category.IsDeleted = !category.IsDeleted
	return srv.Repo.Delete(*category)  //Delete方法是值传递
}
